...
| Commerce Partial Refund In-Flight Committing Visibility RPC Foundation V1 | **COMPLETE / CLOSED** — branch `office/commerce-partial-refund-in-flight-committing-visibility-v1`; local draft **`20260901`** (`list_store_partial_refund_ledger_committing`); admin read-only; **not** remote-applied; verdict **`COMMERCE_PARTIAL_REFUND_IN_FLIGHT_COMMITTING_VISIBILITY_V1_CLOSED`** |

Default: Remote tip **`20260900`** (partial-refund ledger schema + privileged RPCs, service_role only). Local draft **`20260901`** (committing visibility list RPC) is committed on the visibility branch and is **not** remote-applied. Service-role TS adapter + reservation orchestration **CLOSED**. Admin reservation actions + seller reservation **read** wiring **CLOSED**. Accounting audit/review surface **CLOSED**. Stuck-committing recovery **CLOSED**. In-flight committing visibility **CLOSED** (local only). Learning `20260896–97` and payout `20260898` unchanged. Live gate **OFF**. `commerce_confirm` false. Provider refund execution unsupported without a separate GO. Do not modify frozen Commerce architecture documents. Do not delete Store docs.
