...
| Commerce Partial Refund Ledger Service-Role Adapter & Reservation Orchestration V1 | **COMPLETE / CLOSED** — branch `office/commerce-partial-refund-ledger-service-adapter-v1` @ `6a2420e`; verdict **`PARTIAL_REFUND_SERVICE_ADAPTER_V1_CLOSED`**; reservation-only; no provider/money |
| Commerce Partial Refund Reservation Actions & Admin/Seller Wiring V1 | **COMPLETE / CLOSED** — branch `office/commerce-partial-refund-reservation-actions-wiring-v1` @ `5d4bf18`; verdict **`PARTIAL_REFUND_RESERVATION_ACTIONS_WIRING_V1_CLOSED`**; admin reservation + seller read; no money/provider; seller request omitted (policy) |
...
| Commerce Partial Refund Provider Money Execution V1 | **IMPLEMENTATION CLOSED** — branch `office/commerce-partial-refund-provider-money-execution-v1` @ checkpoint `8c6a53e`; state **`PARTIAL_REFUND_PROVIDER_MONEY_EXECUTION_V1_IMPLEMENTATION_CLOSED`**; remote **`20260915`** applied; gates OFF; not production-enabled; Stripe TEST/LIVE activation deferred to coordinator-assigned **Activation & Test Validation V1** (`WAITING_CENTRAL_COORDINATOR_ASSIGNMENT`) |

Default: Commerce compensation RPC live under history **`20260907`** (visibility **`20260905`**; ledger tip chain **`20260899`→`20260900`→`20260905`→`20260907`**). Learning owns **`20260901`** / **`20260906`**. Live gate **OFF**. `commerce_confirm` false. Provider money execution **implementation-closed** (`20260915` remote-applied); gate/mode **OFF**; no Stripe refund executed; Desktop must not self-start activation. Do not modify frozen Commerce architecture documents. Do not delete Store docs.
