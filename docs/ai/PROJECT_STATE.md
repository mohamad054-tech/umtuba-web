...
| Commerce Partial Refund Ledger Service-Role Adapter & Reservation Orchestration V1 | **COMPLETE / CLOSED** — branch `office/commerce-partial-refund-ledger-service-adapter-v1`; verdict **`PARTIAL_REFUND_SERVICE_ADAPTER_V1_CLOSED`**; reservation-only; no provider/money; UI wiring deferred |

Default: Remote tip **`20260900`** (partial-refund ledger schema + privileged RPCs, service_role only). Service-role TS adapter + reservation orchestration **CLOSED**. Learning `20260896–97` and payout `20260898` unchanged. Live gate **OFF**. `commerce_confirm` false. Provider refund execution unsupported without a separate GO. Do not modify frozen Commerce architecture documents. Do not delete Store docs.
