...
| Commerce Partial Refund Reservation Stuck-Committing Recovery V1 | **READY FOR REVIEW** — branch `office/commerce-partial-refund-reservation-stuck-committing-recovery-v1`; admin committing→failed lock release only; no committed cancel/compensation/money |

Default: Remote tip **`20260900`** (partial-refund ledger schema + privileged RPCs, service_role only). Service-role TS adapter + reservation orchestration **CLOSED**. Admin reservation actions + seller reservation **read** wiring **CLOSED**. Accounting audit/review surface **CLOSED**. Stuck-committing recovery **READY FOR REVIEW**. Learning `20260896–97` and payout `20260898` unchanged. Live gate **OFF**. `commerce_confirm` false. Provider refund execution unsupported without a separate GO. Do not modify frozen Commerce architecture documents. Do not delete Store docs.
