...
| Commerce Partial Refund Reservation Actions & Admin/Seller Wiring V1 | **COMPLETE / CLOSED** — branch `office/commerce-partial-refund-reservation-actions-wiring-v1`; verdict **`PARTIAL_REFUND_RESERVATION_ACTIONS_WIRING_V1_CLOSED`**; admin reservation + seller read; no money/provider; seller request omitted (policy) |

Default: Remote tip **`20260900`** (partial-refund ledger schema + privileged RPCs, service_role only). Service-role TS adapter + reservation orchestration **CLOSED**. Admin reservation actions + seller reservation **read** wiring **CLOSED**. Learning `20260896–97` and payout `20260898` unchanged. Live gate **OFF**. `commerce_confirm` false. Provider refund execution unsupported without a separate GO. Do not modify frozen Commerce architecture documents. Do not delete Store docs.
