...
**Status:** PLANNED / WAITING_EXECUTION_GO (inventory complete)  
...
2. **Platform entry:** Desktop primary = Home / World / Learning / Live / Messages; Store + more via user menu + documented circle contracts; Collaboration via `/workspaces` (Collab SoT).  
...
6. **Search:** Top-nav Search → `/search`.  
7. **Map:** World Discovery `/world` (desktop-primary; mobile policy excludes World from bottom nav).  
8. **Notifications:** NotificationBell → `/notifications`.  
9. **Return Home:** UMTUBA mark → `/`.  
...
|----|----------|---------|------------|----------|------|----------|
| P1-1 | Home / Nav | Store & Collaboration weak vs Learning on primary chrome | Shell/nav coherence wave — expose Store (and Collab when gate on) without removing Home feed | avoid Commerce money files | M | YES vs Commerce WAITING |
| P1-2 | Home | Circular arc / HOME_CIRCLE contracts ≠ live Home UI | Document + flag-gated wiring to real hrefs; keep Discover feed | Home lock paths | M | after U1 |
...
| **U1** | Global shell/navigation consistency | AppTopNav/UserMenu/mobile contracts; Store (+ Collab entry when allowed); no Home feed rewrite | NO | NO | Server — **SELECTED FIRST** |
...
| Preserves Home | YES — no DiscoverExperience replacement |
| Preserves Map | YES — no World rewrite |
| Avoids holds | YES — no Commerce money, Translation, Collab migration SQL, Learning bookmarks SQL |
| Tests | nav contract tests under `app/lib/nav/**` |
| Migration | NO |
| Remote DB | NO |

**Do not implement until separate execution GO.**
...
|------------|-------|--------|
| UMTUBA_UI_PRODUCT_UNIFICATION | Central Server | INVENTORY_COMPLETE / WAITING_U1_GO |
