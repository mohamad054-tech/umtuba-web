...
import SectionLifecycleActions from "../../../../components/learning/instructor/SectionLifecycleActions";
import SectionStatusChip from "../../../../components/learning/instructor/SectionStatusChip";
...
export async function generateMetadata({ params }: PageProps) {
  ...
  void sectionId;
  return { title: "Section · Instructor | UM Learning" };
}
...
export default async function InstructorSectionPage({
  ...
  const backHref = course.ok
    ? LEARNING_INSTRUCTOR_ROUTES.course(course.data.id)
    : LEARNING_INSTRUCTOR_ROUTES.hub;
  const backLabel = course.ok ? course.data.name : "Courses";
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-white/50">/{section.data.slug}</p>
        </div>
        <SectionStatusChip status={section.data.status} />
      </div>
