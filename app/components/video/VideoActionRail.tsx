...
export default function VideoActionRail({
  ...
  const [demoStats, setDemoStats] = useState(stats);
  ...
  useEffect(() => {
    if (!persist || !postId) return;

    const supabase = createClient();
    const channel = supabase
      .channel(`watch-post-counters-${postId}`)
      .on(
        "postgres_changes",
        {
          event: "UPDATE",
        ...
        (payload) => {
          const row = payload.new as Partial<DiscoverStats>;
          const patch: Partial<DiscoverStats> = {};
          if (typeof row.likes === "number") patch.likes = row.likes;
          if (typeof row.comments === "number") patch.comments = row.comments;
          if (typeof row.shares === "number") patch.shares = row.shares;
          if (typeof row.saves === "number") patch.saves = row.saves;
          if (typeof row.views === "number") patch.views = row.views;
          if (Object.keys(patch).length > 0) applyRealtimeStats(patch);
        }
      )
      .subscribe();
  ...
  function showCopiedSuccess() {
    ...
    copiedTimerRef.current = window.setTimeout(() => setLinkCopied(false), 2200);
  }
  ...
  async function recordShareAfterSuccess(outcome: SharePostOutcome) {
    if (outcome.method === "none") {
      if (outcome.message !== "Share cancelled.") showHint(outcome.message);
      return;
    }

    setSharedPulse(true);
    window.setTimeout(() => setSharedPulse(false), 420);
    if (outcome.method === "clipboard") showCopiedSuccess();
    else if (outcome.method === "whatsapp") showHint("Opening WhatsApp");
    else showHint("Shared");

    if (!persist || !postId) {
      onStatsChange?.({ shares: demoStats.shares + 1 });
      return;
    }
  ...
  async function handleLike() {
    if (!persist || !postId) {
      const next = !demoLiked;
      setDemoLiked(next);
      setDemoStats((s) => ({
        ...s,
        likes: Math.max(s.likes + (next ? 1 : -1), 0),
      }));
      return;
    }
    ...
    if (!result.ok) {
      ...
      if (result.requiresAuth) redirectToLogin();
      else showHint(result.message);
  ...
  async function handleSave() {
    if (!persist || !postId) {
      const next = !demoSaved;
      setDemoSaved(next);
      setDemoStats((s) => ({
        ...s,
        saves: Math.max(s.saves + (next ? 1 : -1), 0),
      }));
      return;
    }
  ...
  const displayLiked = persist ? likedByMe : demoLiked;
  const displaySaved = persist ? savedByMe : demoSaved;
  const displayStats = persist ? stats : demoStats;
  ...
  return (
    <div className="relative flex flex-col items-center gap-4 pb-1">
      ...
      <ActionButton
        label="Comment"
        count={formatInteractionCount(displayStats.comments)}
      ...
      <div className="relative">
        <ActionButton
          label="Share"
          count={formatInteractionCount(displayStats.shares)}
          active={sharedPulse}
