...
export default function CourseDetailView({ slug }: { slug: string }) {
  ...
  const course = demoCourse(slug) ?? demoCourse("signals-of-thought-ai-studio");
  ...
  return (
    <VisualShell title={t("learning.course.title")} subtitle={loc(course.title, locale)}>
        ...
        <div className="relative min-h-[280px] md:min-h-[380px]">
          ...
          <div className="relative flex min-h-[280px] flex-col justify-end p-6 md:min-h-[380px] md:p-10">
            ...
            <div className="mt-5 flex flex-wrap gap-2 text-xs text-white/70">
              ...
              <span className="rounded-full bg-white/10 px-3 py-1">
                {t("teacher.dashboard.students")} {course.enrollments.toLocaleString(locale === "ar" ? "ar" : "en")}
              </span>
              <span className="rounded-full bg-white/10 px-3 py-1">
                {t("learning.course.durationHours", { values: { hours: course.durationHours } })}
              </span>
              <span className="rounded-full bg-white/10 px-3 py-1">
                {t(LEVEL_KEY[course.level])}
              </span>
              <span className="rounded-full bg-white/10 px-3 py-1">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link
                href={firstLesson ? hrefs.lesson(firstLesson.id) : hrefs.home}
              ...
              >
                {enrolled ? t("learning.course.continue") : t("learning.course.start")}
              </Link>
              <button
                type="button"
                onClick={() => {
                  setEnrolled(true);
                  setToast(t("learning.visual.enrolled"));
                }}
      ...
      {teacher ? (
        ...
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={teacher.portrait} alt="" className="h-14 w-14 rounded-2xl object-cover" />
          <div>
            <p className="font-black">{loc(teacher.name, locale)}</p>
            <p className="text-xs text-white/55">{loc(teacher.bio, locale)}</p>
          </div>
        </Link>
      ) : null}
                ...
                {chapter.lessons.map((lesson) => (
                  <li key={lesson.id}>
                    <Link href={hrefs.lesson(lesson.id)} className="hover:text-white">
                      {loc(lesson.title, locale)} · {lesson.durationMin}m
                    </Link>
                  </li>
                ))}
              </ol>
      ...
      <section className="mt-8">
        ...
        <ul className="space-y-3">
          {course.reviews.map((review) => {
            const student = demoStudent(review.studentId);
            return (
              <li key={review.id} className="rounded-[22px] border border-white/10 bg-white/[0.03] p-4">
                <p className="text-sm font-bold">
                  {student ? loc(student.name, locale) : review.studentId} · {review.rating}
                </p>
