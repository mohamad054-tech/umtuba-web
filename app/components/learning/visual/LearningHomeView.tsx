...
export default function LearningHomeView({
  ...
  const [tab, setTab] = useState<"discover" | "library">(surface);
  ...
  const recommended = DEMO_COURSES.filter((c) => c.category === "ai" || c.category === "uiux");
  const trending = DEMO_COURSES.filter((c) => c.isTrending);
  const free = DEMO_COURSES.filter((c) => c.isFree);
  const fresh = DEMO_COURSES.filter((c) => c.isNew);

  return (
    <VisualShell title={t("learning.hub.title")} subtitle={t("learning.hub.subtitle")}>
        ...
        <div className="space-y-12">
          <section className="overflow-hidden rounded-[32px] border border-white/10 bg-[linear-gradient(135deg,rgba(76,29,149,0.35),rgba(12,18,48,0.9)_48%,rgba(30,64,175,0.28))] p-6 md:p-9">
            ...
            <h1 className="mt-2 text-3xl font-black tracking-tight md:text-5xl">
              {t("learning.visual.greeting", {
                values: { name: loc(DEMO_VIEWER.name, locale) },
              })}
            </h1>
            ...
            <div className="mt-5 flex flex-wrap gap-3 text-xs text-white/70">
              <span className="rounded-full bg-white/10 px-3 py-1.5">
                {t("learning.visual.streak")} · {DEMO_VIEWER.streakDays}
              </span>
              <span className="rounded-full bg-white/10 px-3 py-1.5">
                {t("learning.visual.umPoints")} · {DEMO_VIEWER.umPoints}
              </span>
              <span className="rounded-full bg-white/10 px-3 py-1.5">
          ...
          {resume ? (
            ...
            >
              <div className="p-6 md:p-8">
                ...
                <Link
                  href={hrefs.lesson(resume.enrollment.continueLessonId)}
          ...
          {query || category !== "all" ? (
            <section>
              <SectionTitle title={t("learning.catalog.showing", {
                values: { shown: String(filtered.length), total: String(DEMO_COURSES.length) },
              })} />
          ...
          <section className="rounded-[28px] border border-white/10 bg-white/[0.03] p-6">
            ...
            <p className="mt-4 text-xs text-white/45">
              {LEVEL_KEY.beginner ? t("learning.visual.umPoints") : null} · {DEMO_VIEWER.umPoints}
            </p>
          </section>
        </div>
      )}
