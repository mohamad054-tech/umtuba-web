...
export default function TeacherProfileView({ userId }: { userId: string }) {
  ...
  return (
    <VisualShell title={t("teacher.public.title")} subtitle={loc(teacher.name, locale)}>
      <section className="grid gap-6 overflow-hidden rounded-[32px] border border-white/10 bg-white/[0.04] p-6 md:grid-cols-[220px_1fr] md:p-8">
        ...
        <div>
          ...
          <dl className="mt-5 flex flex-wrap gap-3 text-xs">
            ...
            <div className="rounded-full bg-white/10 px-3 py-1.5">
              {t("teacher.dashboard.students")} {teacher.students.toLocaleString(locale === "ar" ? "ar" : "en")}
            </div>
            <div className="rounded-full bg-white/10 px-3 py-1.5">
              {t("teacher.public.courses")} {teacher.courseCount}
            </div>
          </dl>
          <Link
            href={hrefs.become}
      ...
      <section className="mt-8 rounded-[28px] border border-white/10 bg-white/[0.03] p-6">
        ...
        <ul className="flex flex-wrap gap-2">
          {teacher.achievements.map((item) => (
            <li key={item.en} className="rounded-full bg-violet-500/15 px-3 py-1 text-sm font-bold">
              {loc(item, locale)}
            </li>
          ))}
        </ul>
        <p className="mt-4 text-sm text-white/55">
          {t("learning.review.title")} · {teacher.reviewCount}
        </p>
      </section>
    </VisualShell>
  );
