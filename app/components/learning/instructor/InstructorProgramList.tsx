import Link from "next/link";
import type { InstructorProgramSummary } from "../../../../lib/learning/instructorAuthoring";
import { LEARNING_INSTRUCTOR_ROUTES } from "../../../../lib/learning/instructorAuthoring";
import ProgramStatusChip from "./ProgramStatusChip";

export default function InstructorProgramList({
  spaceId,
  spaceActive,
  programs,
}: {
  spaceId: string;
  spaceActive: boolean;
  programs: InstructorProgramSummary[];
}) {
  return (
    <section className="mt-6 border-t border-white/10 pt-6">
      <div className="mb-3 flex items-center justify-between gap-3">
        <h2 className="text-sm font-bold uppercase tracking-wide text-white/50">
          Programs
        </h2>
        {spaceActive ? (
          <Link
            href={LEARNING_INSTRUCTOR_ROUTES.programNew(spaceId)}
            className="watch-focus-ring rounded-full border border-white/15 bg-white px-3 py-1.5 text-xs font-bold text-black"
          >
            New program
          </Link>
        ) : null}
      </div>

      {!spaceActive ? (
        <p className="rounded-2xl border border-amber-400/25 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
          Publish this space to active before creating programs.
        </p>
      ) : null}

      {programs.length === 0 ? (
        <div className="mt-3 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-5">
          <p className="text-sm text-white/65">No programs yet.</p>
          {spaceActive ? (
            <Link
              href={LEARNING_INSTRUCTOR_ROUTES.programNew(spaceId)}
              className="watch-focus-ring mt-3 inline-flex text-sm font-bold text-white/80 hover:text-white"
            >
              Create the first program →
            </Link>
          ) : null}
        </div>
      ) : (
        <ul className="mt-3 space-y-3">
          {programs.map((program) => (
            <li key={program.id}>
              <Link
                href={LEARNING_INSTRUCTOR_ROUTES.program(program.id)}
                className="watch-focus-ring block rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 transition hover:bg-white/[0.05]"
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-bold text-white">{program.name}</p>
                    <p className="mt-0.5 text-xs text-white/50">
                      /{program.slug}
                    </p>
                  </div>
                  <ProgramStatusChip status={program.status} />
                </div>
                <p className="mt-2 text-xs text-white/45">
                  {program.format.replaceAll("_", " ")} · {program.visibility}
                </p>
              </Link>
            </li>
          ))}
        </ul>
      )}
    </section>
  );
}
