...
type SCard = { s: number; r: number; up: boolean; id: string };

export function SolitaireGame() {
  const { t, locale } = useI18n();
  const help = usePlayHelp();
  const sfx = useMemo(() => createPlaySfx(), []);
  const [tab, setTab] = useState<SCard[][]>([[], [], [], [], [], [], []]);
  const [found, setFound] = useState<SCard[][]>([[], [], [], []]);
  ...
  const deal = () => {
    const deck: SCard[] = [];
    for (let s = 0; s < 4; s += 1) for (let r = 1; r <= 13; r += 1) deck.push({ s, r, up: false, id: `${s}-${r}` });
    const mixed = shuffled(deck);
    const nextTab: SCard[][] = [[], [], [], [], [], [], []];
    for (let c = 0; c < 7; c += 1) {
      for (let n = 0; n <= c; n += 1) {
        const card = mixed.pop();
        if (card) { card.up = n === c; nextTab[c]!.push(card); }
      }
    }
    setTab(nextTab);
    setFound([[], [], [], []]);
  ...
  const drawStock = () => {
    if (stock.length) {
      ...
      setStock((cur) => cur.slice(0, -1));
      setWaste((cur) => [...cur, { ...card, up: true }]);
    } else {
      setStock(waste.slice().reverse().map((card) => ({ ...card, up: false })));
      setWaste([]);
    }
    setMoves((n) => n + 1);
  };
  ...
  const wasteToFound = () => {
    ...
    if (!card) return;
    const pile = found[card.s] ?? [];
    if ((pile.length === 0 && card.r === 1) || (pile[pile.length - 1]?.r === card.r - 1)) {
      setWaste((cur) => cur.slice(0, -1));
      setFound((cur) => cur.map((row, i) => (i === card.s ? [...row, card] : row)));
      setMoves((n) => n + 1);
      sfx.ok();
    }
  };
  ...
  useEffect(() => {
    if (found.every((pile) => pile.length === 13) && found.some((pile) => pile.length)) {
      const pts = Math.max(200, 1200 - moves * 2);
  ...
  if (done) {
    const pts = Math.max(200, 1200 - moves * 2);
    return (
      <PlayPanel stats={<PlayStat label={t("games.moves")} value={formatPlayNumber(locale, moves)} />}>
        <PlayResult score={pts} verdictKey="games.solved" detail={t("games.solitaire.title")} onAgain={() => { deal(); help.keepReadyOnReplay(); }} />
      </PlayPanel>
    );
  }

  return (
    <Shell slug="solitaire" howTo={["games.solitaire.howTo1", "games.solitaire.howTo2", "games.solitaire.howTo3"]} stats={<PlayStat label={t("games.moves")} value={formatPlayNumber(locale, moves)} />} ready={help.ready} helpOpen={help.helpOpen} onToggleHelp={help.toggleHelp} begin={begin}>
      <div className="um-play-klondike" dir="ltr">
        <button type="button" className="um-play-cardmini" data-play-item="true" onClick={drawStock}>{stock.length ? "🂠" : "↺"}</button>
        <button type="button" className="um-play-cardmini" data-play-item="true" onClick={wasteToFound}>{waste[waste.length - 1] ? `${waste[waste.length - 1]!.r}` : " "}</button>
        {tab.map((pile, idx) => (
          <div key={idx} className="um-play-pile">
            {pile.map((card) => (
              <span key={card.id} className="um-play-cardmini" data-play-item="true">{card.up ? card.r : "•"}</span>
            ))}
          </div>
        ))}
      </div>
