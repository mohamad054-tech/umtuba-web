...
const FORBIDDEN_LEARNING_20260934 =
...
describe("seller isolation / RLS contracts", () => {
  ...
  it("does not ship forbidden 20260934 SQL and keeps capture/payout fail-closed in source", () => {
    expect(existsSync(join(ROOT, FORBIDDEN_STORE_20260934))).toBe(false);
    expect(existsSync(join(ROOT, FORBIDDEN_LEARNING_20260934))).toBe(false);
    const migrationNames = readdirSync(join(ROOT, "supabase/migrations"));
    expect(migrationNames.some((name) => name.startsWith("20260934"))).toBe(false);
