...
import { CATEGORY_TAXONOMY_SEED_ID } from "./categoryTaxonomySeed";
import {
...
describe("sellerInventoryAvailabilityFoundation", () => {
  ...
  it("publish readiness compatibility — availability does not replace digital/category gates", () => {
    ...
    expect(CATEGORY_TAXONOMY_SEED_ID).toMatch(/category_taxonomy_seed/);
  });
  ...
  it("preserves taxonomy + digital publish gates and does not touch commerce_confirm", () => {
    ...
    expect(foundation).not.toMatch(/commerce_confirm_enabled/);
    expect(foundation).not.toMatch(/admin_set_commerce_confirm/);

    const cart = read("lib/store/cart.ts");
    expect(cart).toMatch(/resolveTrustedInventoryAvailability/);
    const catalog = read("lib/store/catalogQueries.ts");
    expect(catalog).toMatch(/resolveTrustedInventoryAvailability/);
    const sellerInv = read("lib/store/sellerInventoryQueries.ts");
    expect(sellerInv).toMatch(/resolveTrustedInventoryAvailability/);
  });

  it("documents capability with no new migration", () => {
    const doc = read(
      "docs/store/implementation/SELLER_INVENTORY_AVAILABILITY_FOUNDATION_V1.md"
    );
    expect(doc).toMatch(/commerce\.inventory\.seller_inventory_availability_foundation_v1/);
    expect(doc).toMatch(/No migration/i);
    expect(doc).toMatch(/product_inventory/);
  });
});
