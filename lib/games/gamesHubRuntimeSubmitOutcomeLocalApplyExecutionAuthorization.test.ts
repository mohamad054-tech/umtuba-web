...
const IDEMPOTENCY_KEY = "outcome-local-apply-authorization-1";
...
describe("Games Hub Runtime Submit Outcome Local Apply Execution Authorization Contract Trusted V1", () => {
  it("returns authorized for valid ready guard + accepted-fresh plan", () => {
  ...
  it("fails closed for inconsistent plan/guard metadata", () => {
    const { session, handoff } = boundCompletedPair();
    const plan = trustedPlan(session, handoff);
    const guard = trustedReadyGuard(session, handoff, plan);
  ...
  it("fails closed when prepares* is not true", () => {
    ...
    expect(
      evaluateGamesRuntimeSubmitOutcomeLocalApplyExecutionAuthorizationTrusted(
        ...
        handoff,
        plan,
        forgeGuard(guard, { preparesRuntimeApply: false })
      )
    ).toEqual({ ok: false, reason: "guard_prepares_runtime_apply_required" });
    expect(
      evaluateGamesRuntimeSubmitOutcomeLocalApplyExecutionAuthorizationTrusted(
        session,
        handoff,
        plan,
        forgeGuard(guard, { preparesHandoffApply: false })
      )
    ).toEqual({ ok: false, reason: "guard_prepares_handoff_apply_required" });
  });
  ...
  it("fails closed for guard/runtime identity mismatch", () => {
    const { session, handoff } = boundCompletedPair();
    const plan = trustedPlan(session, handoff);
    const guard = trustedReadyGuard(session, handoff, plan);
  ...
  it("fails closed for plan/guard resultId mismatch", () => {
    const { session, handoff } = boundCompletedPair();
    const plan = trustedPlan(session, handoff);
    const guard = trustedReadyGuard(session, handoff, plan);
    const r =
    ...
    expect(r).toEqual({ ok: false, reason: "plan_guard_result_id_mismatch" });
  });
  ...
  it("returns frozen bounded authorization output", () => {
    ...
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    const auth: GamesRuntimeSubmitOutcomeLocalApplyExecutionAuthorization =
      r.value;
  ...
  it("does not mutate inputs and keeps applied/authority/capability flags false", () => {
    const { session, handoff } = boundCompletedPair();
    const plan = trustedPlan(session, handoff);
    const guard = trustedReadyGuard(session, handoff, plan);
  ...
  it("has no RPC or side effects and keeps Hub authority closed", () => {
    ...
    expect(src).not.toMatch(/grantsCapability:\s*true|providesAuthorityToken:\s*true/);
