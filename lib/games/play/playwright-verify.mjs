...
const EXPECT = {
  ...
  solitaire: { selector: "[data-play-item='true']", min: 2, label: "items" },
...
const lines = [
  ...
  ...counts.map((row) => {
    ...
    const primary = `${row.afterStart[spec.label] ?? 0}/${spec.min} ${spec.label}`;
...
console.log(JSON.stringify({ base, results, directions, counts, catalog }, null, 2));
...
if (failedMount.length || failedDir.length || failedCounts.length || failedCatalog.length) {
  process.exit(1);
}
