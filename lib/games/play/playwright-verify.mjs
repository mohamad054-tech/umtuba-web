...
async function openGame(locale, slug) {
...
for (const locale of locales) {
  for (const slug of slugs) {
    const { page, errors, status } = await openGame(locale, slug);
...
await browser.close();
const report = { base, results, directions };
...
if (failedMount.length || failedDir.length) {
  process.exit(1);
}
