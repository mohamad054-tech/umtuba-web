  ...
  const pileOffsets = await page.locator("[data-tableau]").evaluateAll((piles) =>
    piles.map((pile) => {
      ...
      for (let i = 1; i < stack.length; i += 1) {
        const expected = stack[i - 1].up ? 38 : 12;
        const got = stack[i].top - stack[i - 1].top;
        deltas.push({
          expected,
          got,
          ok: Math.abs(got - expected) <= 2,
          peek: stack[i].peek,
        });
      }
      return { count: stack.length, deltas };
    })
  );
  const offsetOk =
    pileOffsets.length === 7 &&
    pileOffsets.every((pile, index) => pile.count === index + 1 && pile.deltas.every((delta) => delta.ok));
  const actions = (
    await Promise.all([
...
console.log(
  [
    "locale\tviewport\tratio\tinBoard\tranks\tfacedown\toffsets\tbuttons\theader\toverflow\tpass",
    ...solitaireVisual.map(
      (row) =>
        `${row.locale}\t${row.viewport}\t${row.ratioMin.toFixed(2)}-${row.ratioMax.toFixed(2)} ${row.ratioOk}\t${row.inBoard}\t${row.rankOk}\t${row.facedownHidden}\t${row.offsetOk}\t${row.buttonsClear}\t${row.headerOk}\t${row.inBoard}\t${row.pass ? "ok" : "FAIL"}`
    ),
  ].join("\n")
);
console.log(
  JSON.stringify({ base, results, directions, counts, catalog, solitaireMoves, solitaireVisual }, null, 2)
);
