...
console.log(
  [
    "locale\tviewport\tratio\tinBoard\tpeek\tscroll\tchrome\tlong\tpass",
    ...solitaireVisual.map(
      (row) =>
        `${row.locale}\t${row.viewport}\t${row.ratioMin.toFixed(2)}-${row.ratioMax.toFixed(2)} ${row.ratioOk}\t${row.inBoard}\t${row.facePeekOk}\t${row.noPageScroll}\t${row.chromeVisible}\t${row.longPile}\t${row.pass ? "ok" : "FAIL"}`
    ),
  ].join("\n")
);
console.log(
