...
async function assertSolitaireVisual(page, locale, viewport) {
  ...
  return {
    ...
    pass: ratioOk && !overflow && rankOk && noIndic && emptyOk,
  };
}
...
console.log(
  [
    "locale\tviewport\tratio\tinBoard\tranks\tnoIndic\temptyFoundations\tpass",
    ...solitaireVisual.map(
      (row) =>
        `${row.locale}\t${row.viewport}\t${row.ratioMin.toFixed(2)}-${row.ratioMax.toFixed(2)} ${row.ratioOk}\t${row.inBoard}\t${row.rankOk}\t${row.noIndic}\t${row.emptyFoundations}\t${row.pass ? "ok" : "FAIL"}`
    ),
  ].join("\n")
);
console.log(
