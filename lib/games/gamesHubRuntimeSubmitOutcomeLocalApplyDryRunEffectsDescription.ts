/**
 * UM Games Hub Runtime Submit Outcome Local Apply Dry-Run Effects
 * Description Contract Trusted V1 — pure fail-closed dry-run /
 * effects-description only.
 *
 * Describes intended future local effects over an already-trusted local-apply
 * plan, ready precondition guard, and token-less execution authorization after
 * strict continuity checks against the current runtime session and completion
 * handoff, plus exact trusted-plan, trusted-guard, and trusted-authorization
 * consistency.
 *
 * Description metadata only — does not execute apply, mutate runtime session
 * or completion handoff, set handoff.applied to true, change runtime lifecycle
 * state, return a capability / authority token, call Supabase, Submit, Start,
 * or any RPC, open Hub authority, or permit reapply.
 *
 * A successful dry-run description must not be inferred as:
 ...
 *
 * `describesRuntimeApply` / `describesHandoffApply` /
 * `intendedRuntimeEffect` / `intendedHandoffEffect` are description metadata
 * only and must not be inferred as apply authority.
 *
 * SQL remains the sole submit decision and mutation authority.
 * Existing trusted local-apply plan, ready precondition guard, and token-less
 * execution authorization are the only trusted classification inputs.
...
/**
 * Bounded intended Runtime local-apply effect descriptions.
 * Description metadata only — not execution capability.
 */
export const GAMES_RUNTIME_SUBMIT_OUTCOME_LOCAL_APPLY_DRY_RUN_INTENDED_RUNTIME_EFFECTS =
  ["mark_runtime_completion_locally"] as const;

export type GamesRuntimeSubmitOutcomeLocalApplyDryRunIntendedRuntimeEffect =
  (typeof GAMES_RUNTIME_SUBMIT_OUTCOME_LOCAL_APPLY_DRY_RUN_INTENDED_RUNTIME_EFFECTS)[number];

/**
 * Bounded intended handoff local-apply effect descriptions.
 * Description metadata only — not execution capability.
 */
export const GAMES_RUNTIME_SUBMIT_OUTCOME_LOCAL_APPLY_DRY_RUN_INTENDED_HANDOFF_EFFECTS =
  ["mark_completion_handoff_applied_locally"] as const;

export type GamesRuntimeSubmitOutcomeLocalApplyDryRunIntendedHandoffEffect =
  (typeof GAMES_RUNTIME_SUBMIT_OUTCOME_LOCAL_APPLY_DRY_RUN_INTENDED_HANDOFF_EFFECTS)[number];

/**
 * Immutable Hub Runtime local-apply dry-run effects-description view.
 *
 * Description metadata only — not apply execution, lifecycle transition,
 * handoff mutation, reapply permission, progress/achievement mutation,
 * reward/economy entitlement, Hub synchronization, capability token,
 * authority token, or an executor/callback.
 *
 * `describesRuntimeApply` / `describesHandoffApply` /
 * `intendedRuntimeEffect` / `intendedHandoffEffect` are description markers
 * only. `dryRun` is always literal `true`. Authority / execution / capability
 * flags are always literal `false`. A successful dry-run description is not
 * an executor and not execution permission.
 */
export type GamesRuntimeSubmitOutcomeLocalApplyDryRunEffectsDescription =
  Readonly<{
...
/**
 * Exact trusted-plan consistency required for dry-run description.
 */
...
/**
 * Exact trusted-guard consistency required for dry-run description.
 */
...
/**
 * Exact trusted-authorization consistency required for dry-run description.
 */
...
function freezeDryRunEffectsDescription(input: {
  runtimeSessionId: string;
  gameId: string;
  playerId: string;
  platformSessionId: string;
  ...
  return Object.freeze({
    ...
    intendedRuntimeEffect: "mark_runtime_completion_locally" as const,
    intendedHandoffEffect: "mark_completion_handoff_applied_locally" as const,
...
/**
 * Pure fail-closed describer: trusted local-apply plan + ready precondition
 * guard + token-less authorized execution authorization + current runtime /
 * handoff → dry-run effects-description view.
 *
 ...
 * session, completion handoff, trusted plan, trusted guard, and trusted
 * authorization; plus plan.resultId === guard.resultId ===
 ...
 * call RPC. A successful dry-run description is not an executor and not
 * execution permission.
 */
export function describeGamesRuntimeSubmitOutcomeLocalApplyDryRunEffectsTrusted(
  ...
  return {
    ok: true,
    value: freezeDryRunEffectsDescription({
