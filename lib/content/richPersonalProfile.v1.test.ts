...
describe("rich personal profile foundation v1", () => {
  ...
  it("keeps the approved editor on /settings and rich sections on About", () => {
    ...
    expect(settings).toMatch(/CommunicationsPrivacyPanel/);
    ...
    expect(about).toMatch(/profile\.about\.places/);
    expect(about).toMatch(/profile\.rich\?\.places/);
  });
