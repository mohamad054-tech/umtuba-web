...
export type SandboxLessonExercise = {
  title: string;
  prompt: string;
};
...
export type SandboxCertificateCopy = {
  courseName: string;
  learnerNamePlaceholder: string;
  completionStatement: string;
  issuer: "UMTUBA";
  completionDatePlaceholder: string;
  certificateIdPlaceholder: string;
  disclaimer: string;
};

/**
 * Content mapping for one course-level score-mode activity.
 * Does not invent a new Learning backend: maps onto
 * `learning_activities` + `learning_activity_settings`
 * (`completion_mode='score'`, `passing_score`, `max_attempts`).
 */
export type SandboxFinalAssessment = {
  ...
  quiz: QuizQuestion[];
  estimatedMinutes: number;
  /** Maps to learning_activity_settings.max_score (points). */
  maxScore: number;
  /** Maps to learning_activity_settings.passing_score (points; must be <= maxScore). */
  passingScore: number;
  /**
   * Maps to learning_activity_settings.max_attempts.
   * null = unlimited retries (a new attempt may start until a live cap is set).
   */
  maxAttempts: number | null;
  completionMode: "score";
  completionRelationship: string;
};
...
export type SandboxCourse = {
  ...
  publicCatalog: false;
  synthetic: true;
  certificateCopy?: SandboxCertificateCopy;
  finalAssessment?: SandboxFinalAssessment;
};

export type UmtubaOriginalCourse = SandboxCourse & {
  certificateCopy: SandboxCertificateCopy;
  finalAssessment: SandboxFinalAssessment;
};
