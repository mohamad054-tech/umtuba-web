...
describe("data export migration security", () => {
  ...
  it("queues requests and does not generate archives", () => {
    expect(sql).toMatch(
      /create table if not exists public\.data_export_requests/
    );
    expect(sql).toMatch(/Does not generate archives/i);
    expect(sql).not.toMatch(/copy to/i);
  });
