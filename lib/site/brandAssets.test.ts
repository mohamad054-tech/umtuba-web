...
describe("approved video logo assets", () => {
  ...
  it("keeps header frames overflow-visible and object-contain (no cover/clip)", () => {
    for (const preset of Object.values(BRAND_MARK_PRESETS)) {
      expect(preset.frameClassName).toContain("umtuba-brand-frame");
      expect(preset.className).toContain("umtuba-brand-mark");
    }
    expect(BRAND_MARK_PRESETS.nav.maxDisplayPx).toBeLessThanOrEqual(56);
  });
