...
describe("AI Usage, Quotas & Billing Foundation V1", () => {
  ...
  it("integrates metering from Capability Catalog", () => {
    const entry = getCapabilityCatalogRegistry().lookup(
      "commerce.product_draft_assistant"
    );
    expect(entry?.metering.quotaPolicyId).toBeTruthy();
    expect(entry?.metering.budgetPolicyId).toBeTruthy();
    expect(entry?.metering.estimationPolicyId).toBeTruthy();
    expect(entry?.metering.usageUnitType).toBe("request");
  });
