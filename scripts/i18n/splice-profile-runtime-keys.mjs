...
import {
  PROFILE_RUNTIME_CATALOGS,
  PROFILE_RUNTIME_KEYS,
} from "./profileRuntimeCatalog.mjs";
...
function formatEntries(catalog) {
  return PROFILE_RUNTIME_KEYS.map((key) => {
...
function spliceLocaleFile(locale, relativePath) {
  ...
  const block = `${formatEntries(PROFILE_RUNTIME_CATALOGS[locale])}\n  `;
  if (source.includes('"profile.creatorSpace"')) {
    console.log(`skip existing keys: ${relativePath}`);
    return;
  }
  writeFileSync(path, source.slice(0, lineStart) + block + source.slice(lineStart));
}
...
function spliceTypes() {
  ...
  const lineStart = source.lastIndexOf("\n", index) + 1;
  const block = PROFILE_RUNTIME_KEYS.map(
    (key) => `  ${tsString(key)}: string;`
  ).join("\n");
  writeFileSync(
    path,
    `${source.slice(0, lineStart)}${block}\n  ${source.slice(lineStart)}`
  );
}
...
function spliceFoundation() {
  ...
  for (const key of PROFILE_RUNTIME_KEYS) {
    json[key] = PROFILE_RUNTIME_CATALOGS.en[key];
  }
  writeFileSync(path, `${JSON.stringify(json, null, 2)}\n`);
}

for (const key of PROFILE_RUNTIME_KEYS) {
  for (const [locale, catalog] of Object.entries(PROFILE_RUNTIME_CATALOGS)) {
    if (!catalog[key]) {
...
spliceFoundation();
console.log(`spliced ${PROFILE_RUNTIME_KEYS.length} profile keys into 13 catalogs`);
