...
import { COLLABORATION_LEARNING_LINK_E2E_TESTIDS } from "../../../lib/collaboration/learningResourceLinkUnlinkE2eProvisioning";

/**
 * Gate-off / shell readiness probes.
 * Skips unless COLLABORATION_E2E=1 and PLAYWRIGHT_BASE_URL are present.
 * Does not mutate Auth users or create resource links.
 */
...
test.describe("collaboration platform gate readiness", () => {
  test.skip(
    !e2eOn || !hasBase,
    "Requires COLLABORATION_E2E=1 and PLAYWRIGHT_BASE_URL"
  );
  ...
  test("gate-off: /workspaces is not found when platform disabled", async ({
    ...
    if (status === 200) {
      const shell = page.getByTestId(
        COLLABORATION_LEARNING_LINK_E2E_TESTIDS.shell
      );
  ...
  test("gate-on optional: shell test id present when platform enabled", async ({
    ...
    await expect(
      page.getByTestId(COLLABORATION_LEARNING_LINK_E2E_TESTIDS.shell)
    ).toBeVisible({ timeout: 15_000 });
    await expect(
      page.getByTestId(COLLABORATION_LEARNING_LINK_E2E_TESTIDS.nav)
    ).toBeVisible();
  });
});
